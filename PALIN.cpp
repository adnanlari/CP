...
int main()
{
	...
	while(t--)
	{
		...
		if(l%2==0)
		{
			...
			if(test==1)
			{
				for(ll i=0;i<=left;i++) cout<<s[i];
				for(ll i=left;i>=0;i--) cout<<s[i];
				cout<<endl;
			}
			if(test==0)
			{
				...
				for(ll i=0;i<=left;i++) cout<<s[i];if(x==-1) left--;
				for(ll i=left;i>=0;i--) cout<<s[i];
				if(x==-1) { cout<<"1";}
				cout<<endl;
			}
		...
		else if(l%2==1)
		{
			...
			if(test==0)
			{
				...
				while(s[x]=='9' ){ s[x]='0';x--;if(x==-1) break;}
				if(x==-1) { cout<<"1";} else { s[x]=(char)(s[x]+1);}
				for(ll i=0;i<=mid;i++) cout<<s[i];if(x==-1) left--;
				for(ll i=left;i>=0;i--) cout<<s[i];
				if(x==-1) { cout<<"1";}
				cout<<endl;
			}
